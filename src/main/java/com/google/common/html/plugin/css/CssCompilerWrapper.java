...
final class CssCompilerWrapper {
  ...
  boolean compileCss(final Log log) throws IOException {
    ...
    final class Compiler extends ClosureCommandLineCompiler {
      Compiler(
          JobDescription job,
          ExitCodeHandler exitCodeHandler,
          ErrorManager errManager) {
        super(job, exitCodeHandler, errManager);
      }

      @Override
      public String execute(
          @Nullable File renameFilePath, @Nullable File sourceMapFilePath) {
        return super.execute(renameFilePath, sourceMapFilePath);
      }
    }

    String compiledCss = new Compiler(job, exitCodeHandler, errorManager)
